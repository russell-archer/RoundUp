function insert(item, user, request) {
    ...
    sessionTable.where({ id: item.sid }).read({
        success: function (rows) {
            ...
            sessionTable.update(sessionRow, {
                success: function () {
                    ...
                    request.execute({
                        success: function () {
                            ...
                            // Send MPNS "InviteeHasAccepted" notifications to both the invitee and inviter
                            // First, notify the invitee

                            var payloadMsgInvitee =
                            "{" +
                            "  id:" + -1 + "," +
                            "  Recipient:" + 1 + "," +
                            ...
                            "  Latitude:" + sessionRow.Latitude.toString() + "," +
                            "  Longitude:" + sessionRow.Longitude.toString() +
                            "}";
                            ...
                            tables.getTable("RoundUpNotification").insert(notificationInvitee, {
                                error: function (err) {
                                    ...
                                    console.error(err + " RoundUpNotification insert failed (101)");
                                }
                            });
                            
                            push.mpns.sendRaw(item.Channel, { payload: payloadMsgInvitee }, {
                                
                                success: function() {

                                    console.log("MPNS: InviteeHasAccepted sent to invitee. InviteeId = " + item.id + ", SessionId = " + item.sid);
                                    notifyInviter(item, request, sessionRow.Channel);
                                },
                                error: function (err) {
                                    ...
                                    if (isRealMpnsError(err)) {
                                        ...
                                        logMpnsResult(err, "InviteeHasAccepted failed (102)");
                                    ...
                                    // Not a real MPNS error, so notify the inviter
                                    notifyInviter(item, request, sessionRow.Channel);
                                }
                            });
                        },
                        ...
                        error: function (err) {
                            ...
                            console.error(err + " Invitee row update failed (105)");
                ...
                error: function (err) {
                    ...
                    console.error(err + " Invitee insert failed: can't commit changes to Session (106)");
                    request.respond(statusCodes.BAD_REQUEST, "ERR_INSERT_FAILED");
                    return;
                }
            });
        ...
        error: function (err) {
            ...
            console.error(err + " Invitee insert failed: can't read Session (107)");
            request.respond(statusCodes.BAD_REQUEST, "ERR_READ_FAILED");
            return;
        }
    });
...
function notifyInviter(item, request, inviterChannel) {
    ...
    // Notify the inviter
    push.mpns.sendRaw(inviterChannel, { payload: payloadMsgInviter }, {
        success: function () {
        ...
        error: function (err) {
            ...
            if (isRealMpnsError(err)) {
                ...
                logMpnsResult(err, "InviteeHasAccepted failed (104)");
